...
const Login: React.FC = () => {
    ...
    const [email, setEmail] = useState('admin@visulab.com');
        ...
        try {
            ...
            if (error) {
                ...
                if (error.message.includes('already registered')) {
                    alert('O usuário admin@visulab.com já existe. Tente fazer login.');
                } else {
                    throw error;
                }
            } else if (data.user) {
                if (data.session) {
                    alert('Usuário criado e logado com sucesso! O banco de dados deve ter sido populado pelo Trigger. Tente entrar.');
                    navigate('/dashboard');
                } else {
                    alert('Usuário criado, mas sem sessão ativa. Verifique se a confirmação de email está DESATIVADA no Supabase. Se estiver ativada, confirme seu email.');
                }
            }
        } catch (error: any) {
                ...
                <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 md:p-10 transition-all duration-300">
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100 text-center">
                        ...
                        <button
                            ...
                            className="mt-4 text-[10px] text-slate-300 hover:text-primary transition-colors"
                        >
                            (Dev) Criar Admin
                        </button>
                    </div>
